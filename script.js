...
function createEarth() { /* ... function is unchanged ... */ const earthGeometry = new THREE.SphereGeometry(2, 32, 32); const earthMaterial = new THREE.MeshPhongMaterial({ color: 0x4444ff, shininess: 100 }); earth = new THREE.Mesh(earthGeometry, earthMaterial); earth.position.set(EARTH_DISTANCE, 0, 0); scene.add(earth); createOrbit(EARTH_DISTANCE, 0x4444ff); }
...
function createAsteroid(asteroidData) { /* ... function is unchanged ... */ const size = parseFloat(asteroidData.estimated_diameter.kilometers.estimated_diameter_max); const distance = parseFloat(asteroidData.close_approach_data[0].miss_distance.kilometers) * DISTANCE_SCALE; const velocity = parseFloat(asteroidData.close_approach_data[0].relative_velocity.kilometers_per_second); let color = size < 0.1 ? 0x00ff00 : (size < 0.5 ? 0x0088ff : 0xff0000); const asteroidRadius = Math.max(size * SIZE_SCALE, 0.2); const finalRadius = size < 0.01 ? Math.max(asteroidRadius, 0.5) : asteroidRadius; const asteroidGeometry = new THREE.SphereGeometry(finalRadius, 16, 16); const asteroidMaterial = new THREE.MeshPhongMaterial({ color: color, shininess: 30 }); const asteroid = new THREE.Mesh(asteroidGeometry, asteroidMaterial); const angle = Math.random() * Math.PI * 2; const orbitRadius = Math.max(distance, EARTH_DISTANCE + 20); asteroid.position.set(Math.cos(angle) * orbitRadius, (Math.random() - 0.5) * 20, Math.sin(angle) * orbitRadius); asteroid.userData = { ...asteroidData, orbitRadius, angle, angularVelocity: 0.001 / Math.sqrt(orbitRadius), size, velocity }; if (size > 0.05) { createOrbit(orbitRadius, color); } scene.add(asteroid); asteroidObjects.push(asteroid); return asteroid; }
...
function animate() {
    ...
    if (!isPaused) {
        ...
        asteroidObjects.forEach(a => {
            ...
            a.position.x = Math.cos(a.userData.angle) * a.userData.orbitRadius;
            a.position.z = Math.sin(a.userData.angle) * a.userData.orbitRadius;
        });
        if (trackedAsteroid) {
...
function addEventListeners() {
    ...
    document.getElementById('reset-view').addEventListener('click', () => { trackedAsteroid = null; camera.position.set(0, 100, 300); controls.target.set(0, 0, 0); });
